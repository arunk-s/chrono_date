...
/**
* Made an inline version of to_days equivalent function of prototype
*
*/
namespace boost
{
...
void years_days_in_civil()
{
  ...
  std::cout << "Converting dates with days_from_civil() (without casting)               " << t1-t0<< " Total Years " << ycount << '\n';
}
...
void ymd_years_days_in_civil()
{
  ...
  int ycount = 0;
  Clock::time_point t0 = Clock::now();
  boost::chrono::days_rep prev = days_from_civil(Ymin,1,1) - 1 ;
  for (int y = Ymin; y <=Ymax; ++y)
  {
    for (int m = 1; m <=12; ++m)
      {
  ...
  Clock::time_point t1 = Clock::now();
  std::cout << "Converting dates from days_from_civil() with casting                    " << t1-t0 << " Total Years " << ycount <<" " <<count << '\n';
}
...
void years_days_in_civil_opaque()
{
  ...
  Clock::time_point t1 = Clock::now();
  std::cout << "Converting dates from days_from_civil() (opaque)                        " << t1-t0<< " Total Years " << ycount << " " <<count << '\n';
}
...
void serial_date_days_date()
{
  ...
  boost::int_least64_t prev = dt.days_since_epoch().count() - 1 ;
  for (int y = Ymin; y <=Ymax; ++y)
    {
    for (int m = 1; m <=12; ++m)
      {
  ...
  Clock::time_point t1 = Clock::now();
  std::cout << "Converting dates from days_since_epoch()                                " << t1-t0 << " Total Years " << ycount <<" " <<count << '\n';
}
/**
 * Passing days from 1900/1/1 to 4100/1/1 to the algorithm
 * directly in integer
 * But using to_days (inlined) equivalent function of prototype
 **/
void ymd_to_days_inline()
{
  ...
  Clock::time_point t1 = Clock::now();
  std::cout << "Converting dates from to_days2()                                        " << t1-t0<< " Total Years " << ycount << " " <<count << '\n';
}
/**
 * Passing days from 1900/1/1 to 4100/1/1 to the algorithm
 * directly in integer
 * But using equivalent to_days() from prototype
 **/
void ymd_to_days()
{
  boost::chrono::days_rep count = 0;
  ...
  Clock::time_point t1 = Clock::now();
  std::cout << "Converting dates from to_days()                                         " << t1-t0<< " Total Years " << ycount << " " <<count << '\n';
}
